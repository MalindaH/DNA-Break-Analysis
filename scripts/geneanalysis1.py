        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                if(dash > colon+1): # disregard those without position info
                    ...
                    with open(anno_folder+"/chr"+str(chr_num)+"_repair-genes.txt", "a+") as outputf:
                        outputf.write(f'chr{chr_num}\t{start}\t{end}\t{row[0]}\n')
                line_count += 1
    x = 1
    y = ''
    while x <= 22 or y=='X':
        if y == 'X':
            x = 'X'
        if os.path.exists(anno_folder+"/chr"+str(x)+"_repair-genes.txt"):
            with open(anno_folder+"/chr"+str(x)+"_repair-genes.txt", "r") as unsorted:
                sorted = open(anno_folder+"/chr"+str(x)+"_repair-genes-sorted.txt", "a+")
                l = []
                arr = []
                for row in unsorted:
                    l.append(row)
                ...
                sorted.close()
                os.remove(anno_folder+"/chr"+str(x)+"_repair-genes.txt")
        if(x == 22):
            y = 'X' # only chrX has repair genes in this file
        if(x=='X'):
            break
        x += 1
...
# move windows of csv file
def alignto_cancer_genes(chrnum):
    if os.path.exists(anno_folder+"/chr"+str(chrnum)+"_repair-genes-sorted.txt"):
        with open(anno_folder+"/chr"+str(chrnum)+"_repair-genes-sorted.txt", "r") as genes:
...
def calc_pval(chrnum):
    if no_control == '0': # with control:
        ...
        N = k + n
        output = open(output_folder+"/chr"+str(chrnum)+"_sensitive-repair-genes.txt", "a+")
...
def rank_output():
    if os.path.exists('output/chr1_sensitive-repair-genes.txt'):
        filenames = ['output/chr1_sensitive-repair-genes.txt', 'output/chr2_sensitive-repair-genes.txt', 'output/chr3_sensitive-repair-genes.txt',
            'output/chr4_sensitive-repair-genes.txt', 'output/chr5_sensitive-repair-genes.txt', 'output/chr6_sensitive-repair-genes.txt', 
            'output/chr7_sensitive-repair-genes.txt', 'output/chr8_sensitive-repair-genes.txt', 'output/chr9_sensitive-repair-genes.txt', 
            'output/chr10_sensitive-repair-genes.txt', 'output/chr11_sensitive-repair-genes.txt', 'output/chr12_sensitive-repair-genes.txt', 
            'output/chr13_sensitive-repair-genes.txt', 'output/chr14_sensitive-repair-genes.txt', 'output/chr15_sensitive-repair-genes.txt', 
            'output/chr16_sensitive-repair-genes.txt', 'output/chr17_sensitive-repair-genes.txt', 'output/chr18_sensitive-repair-genes.txt', 
            'output/chr19_sensitive-repair-genes.txt', 'output/chr20_sensitive-repair-genes.txt', 
            'output/chr22_sensitive-repair-genes.txt', 'output/chrX_sensitive-repair-genes.txt']
        with open('output/allchr_sensitive-repair-genes-temp.txt', 'w+') as tfile:
            for fname in filenames:
                with open(fname) as infile:
                    tfile.write(infile.read())
        for fname in filenames:
            os.remove(fname)
        tempfile = open('output/allchr_sensitive-repair-genes-temp.txt', 'r')
        outfile = open('output/allchr_sensitive-repair-genes-sorted.txt', 'a+')
        for line in sorted(tempfile, key=lambda line: float(line.split()[0])):
            outfile.write(line)
        tempfile.close()
        os.remove('output/allchr_sensitive-repair-genes-temp.txt')
        outfile.close()


#print("Usage: python geneanalysis.py output annotation_files outputtest0827 ../genome-annotation/DNA_repair_genes_Lucia.csv temp $no_control ../genome-annotation/refGene.txt")
print("\n-> Analyzing sensitive DNA repair genes......")
output_folder = sys.argv[1]
anno_folder = sys.argv[2]
output_name = sys.argv[3]
anno_repair_file = sys.argv[4]
temp_folder = sys.argv[5]
no_control = sys.argv[6]
anno_refgene = sys.argv[7]


## <- find sensitive DNA repair genes, from Lucia (DNA_repair_genes_Lucia.csv) -> ##

# only need to run this method once
process_cancer_genes()

xs = list(range(1,23))
xs.remove(21) # repair genes file doesn't contain chr21
xs.append('X') # only chrX has cancer genes in cancer file

i=0
for x in xs:
    update_progress(i/22, x)
