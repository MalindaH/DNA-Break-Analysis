...
def calc_pval(chrnum):
  if no_control == '0': # with control:
    ...
    output = open(temp_folder+'/chr'+str(chrnum)+'_pval.txt', 'a+')
...
def calc_qval(chrnum):
  plist = []
  with open(temp_folder+'/chr'+str(chrnum)+'_pval.txt', 'r') as pfile:
  ...
  with open(temp_folder+'/chr'+str(chrnum)+'_pval.txt', 'r') as pfile:
    for line in pfile:
      ...
      index += 1
  output.close()
  os.remove(temp_folder+'/chr'+str(chrnum)+'_pval.txt')
